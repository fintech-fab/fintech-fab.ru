	...
	public function getChannels()
	{
		...
		foreach ($aChannels as $iKey => $sChannelName) {
			if (strpos($sChannelName, 'мобильный')) {
				...
				$aChannelsList[$iKey] = '<span data-card="1" rel="tooltip" data-placement="right" data-title="' . AdminKreddyApiComponent::C_CARD_WARNING_COMMISSION . '" title="' . AdminKreddyApiComponent::C_CARD_WARNING_COMMISSION . '">' . $sChannelName . '</span>';
			}
		}
		if (!empty($sMobileChannels) && !empty($sMobileChannelName)) {
			$aChannelsList[$sMobileChannels] = '<span data-card="0" rel="tooltip"  data-placement="right" data-title="' . AdminKreddyApiComponent::C_MOBILE_MSG_REQUIREMENTS . '" title="' . AdminKreddyApiComponent::C_MOBILE_MSG_REQUIREMENTS . '">' . SiteParams::mb_ucfirst(self::formatChannelName($sMobileChannelName)) . '</span>';
		}
