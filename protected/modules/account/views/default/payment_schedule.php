...
?>
	...
	<div class="alert alert-warning">
		...
		<p style="display: none;" id="detail_info">
			...
			Проценты: <?= $aPaymentData['percent_balance'] ?> руб. <br /> -
	...
	<div class="alert alert-warning" style="float:left; width: 53%;">
		<?php if ($aPaymentData['subscription_balance'] > 0) { ?>
			<h5><strong>Абонентская плата за сервис: <?= $aPaymentData['subscription_balance'] ?> руб.</strong></h5>
		<?php } else { ?>
			<h5><strong>Абонентская плата за сервис внесена</strong></h5>
		<?php } ?>
		<?php if ($aPaymentData['penalty_balance'] > 0) { ?>
...
<?php if ($iLoanTotal > 0) { ?>
	...
	<div class="alert alert-warning" style="float:left; width: 53%;">
		<h5><strong>Мы перевели тебе: <?= $aPaymentData['loan_amount'] ?> руб.</strong></h5>
		<h5><strong>Ты вернул: <?= $aPaymentData['loan_amount'] - $aPaymentData['loan_balance'] ?> руб.</strong></h5>
		...
		<?php if ($aPaymentData['fine_balance'] > 0) { ?>
