...
(function (c) {
	c.fn.styler = function (d) {
		d = c.extend({wrapper: "form", idSuffix: "-styler", filePlaceholder: "\u0424\u0430\u0439\u043b \u043d\u0435 \u0432\u044b\u0431\u0440\u0430\u043d", fileBrowse: "\u041e\u0431\u0437\u043e\u0440...", selectSearch: !0, selectSearchLimit: 10, selectSearchNotFound: "\u0421\u043e\u0432\u043f\u0430\u0434\u0435\u043d\u0438\u0439 \u043d\u0435 \u043d\u0430\u0439\u0434\u0435\u043d\u043e", selectSearchPlaceholder: "\u041f\u043e\u0438\u0441\u043a...", selectVisibleOptions: 0, singleSelectzIndex: "100",
			selectSmartPositioning: !0, onSelectOpened: function () {
		...
		return this.each(function () {
			...
			a.is(":checkbox") ? a.each(function () {
				if (1 > a.parent("div.jq-checkbox").length) {
					var d = function () {
						var d = new s, b = c("<div" + d.id + ' class="jq-checkbox' + d.classes + '"' + d.title + '><div class="jq-checkbox__div"></div></div>');
						a.css({position: "absolute", zIndex: "-1", opacity: 0, margin: 0, padding: 0}).after(b).prependTo(b);
						b.attr("unselectable", "on").css({"-webkit-user-select": "none", "-moz-user-select": "none", "-ms-user-select": "none", "-o-user-select": "none", "user-select": "none", display: "inline-block",
				...
				if (1 > a.parent("div.jq-radio").length) {
					var p = function () {
						var q = new s, b = c("<div" + q.id + ' class="jq-radio' + q.classes + '"' + q.title + '><div class="jq-radio__div"></div></div>');
						a.css({position: "absolute", zIndex: "-1", opacity: 0, margin: 0, padding: 0}).after(b).prependTo(b);
						b.attr("unselectable",
							"on").css({"-webkit-user-select": "none", "-moz-user-select": "none", "-ms-user-select": "none", "-o-user-select": "none", "user-select": "none", display: "inline-block", position: "relative"});
						a.is(":checked") && b.addClass("checked");
						a.is(":disabled") && b.addClass("disabled");
						b.click(function () {
			...
			}) : a.is(":file") ? a.css({position: "absolute", top: 0, right: 0, width: "100%", height: "100%", opacity: 0, margin: 0, padding: 0}).each(function () {
				if (1 > a.parent("div.jq-file").length) {
					var p = function () {
						var q = new s, b = c("<div" + q.id +
							' class="jq-file' + q.classes + '"' + q.title + ' style="display: inline-block; position: relative; overflow: hidden"></div>'), p = c('<div class="jq-file__name">' + d.filePlaceholder + "</div>").appendTo(b);
						...
						a.change(function () {
							...
							"" == a.val() && p.text(d.filePlaceholder)
						}).focus(function () {
							b.addClass("focused")
						}).blur(function () {
							b.removeClass("focused")
					...
					var p = function () {
						...
						function b() {
							...
							for (len = g.length; i < len; i++) {
								...
								g.eq(i).parent().is("optgroup") && (void 0 !== g.eq(i).parent().attr("class") && (d = " " + g.eq(i).parent().attr("class")),
									a = "<li" + h + ' class="' + c + b + " option" + d + '">' + g.eq(i).text() + "</li>", g.eq(i).is(":first-child") && (a = '<li class="optgroup' + d + '">' + g.eq(i).parent().attr("label") + "</li>" + a));
								w += a
							}
						}

						function p() {
							var r = new s, e = c("<div" + r.id + ' class="jq-selectbox jqselect' + r.classes + '" style="display: inline-block; position: relative; z-index:' + d.singleSelectzIndex + '"><div class="jq-selectbox__select"' + r.title + ' style="position: relative"><div class="jq-selectbox__select-text"></div><div class="jq-selectbox__trigger"><div class="jq-selectbox__trigger-arrow"></div></div></div></div>');
							a.css({margin: 0, padding: 0}).after(e).prependTo(e);
							var r = c("div.jq-selectbox__select", e), m = c("div.jq-selectbox__select-text", e), k = g.filter(":selected");
							...
							var f = c('<div class="jq-selectbox__dropdown" style="position: absolute">' +
								h + '<ul style="position: relative; list-style: none; overflow: auto; overflow-x: hidden">' + w + "</ul></div>");
							e.append(f);
							var n = c("ul", f), l = c("li", f), t = c("input", f), x = c("div.jq-selectbox__not-found", f).hide();
							...
							var h = e.clone().appendTo("body").width("auto"), v = h.width();
							h.remove();
							v ==
								e.width() && (m.width(A), u += e.find("div.jq-selectbox__trigger").width());
							u > e.width() && f.width(u);
							a.css({position: "absolute", left: 0, top: 0, width: "100%", height: "100%", opacity: 0});
							var B = e.outerHeight(), z = t.outerHeight(), y = n.css("max-height"), h = l.filter(".selected");
							...
							"auto" == f.css("top") && f.css({top: B});
							f.hide();
							h.length && (g.first().text() !=
								k.text() && e.addClass("changed"), e.data("jqfs-class", h.data("jqfs-class")), e.addClass(h.data("jqfs-class")));
							...
							r.click(function () {
								...
								if (!navigator.userAgent.match(/(iPad|iPhone|iPod)/g)) {
									if (d.selectSmartPositioning) {
										var b = c(window), g = e.offset().top, m = b.height() - B - (g - b.scrollTop()), h = d.selectVisibleOptions, k = l.data("li-height"), p = 5 *
											k, r = k * h;
										0 < h && 6 > h && (p = r);
										0 == h && (r = "auto");
										m > p + z + 20 ? (f.height("auto").css({bottom: "auto", top: C}), h = function () {
											n.css("max-height", Math.floor((m - 20 - z) / k) * k)
										}, h(), n.css("max-height", r), "none" != y && n.css("max-height", y), m < f.outerHeight() + 20 && h()) : (f.height("auto").css({top: "auto", bottom: C}), h = function () {
									...
									c("div.jqselect").css({zIndex: d.singleSelectzIndex -
										1}).removeClass("opened focused");
									e.css({zIndex: d.singleSelectzIndex});
									f.is(":hidden") ? (c("div.jq-selectbox__dropdown:visible").hide(), f.show(), e.addClass("opened"), d.onSelectOpened.call(e)) : (f.hide(), e.removeClass("opened"), c("div.jq-selectbox").filter(".opened").length && d.onSelectClosed.call(e));
							...
							l.filter(":not(.disabled):not(.optgroup)").click(function () {
								...
								if (D != n) {
									...
									D = n;
									m.html(n);
									g.first().text() != n ? e.addClass("changed") : e.removeClass("changed");
							...
							c(document).on("click", function (a) {
								c(a.target).parents().hasClass("jq-selectbox") || "OPTION" == a.target.nodeName ||
								(c("div.jq-selectbox").filter(".opened").length && d.onSelectClosed.call(c("div.jq-selectbox").filter(".opened")), t.length && t.val("").keyup(), f.hide().find("li.sel").addClass("selected"), e.removeClass("focused opened"))
							})
						}

						function E() {
							var d = new s, e = c("<div" + d.id + ' class="jq-select-multiple jqselect' + d.classes + '"' + d.title + ' style="display: inline-block; position: relative"></div>');
							a.css({margin: 0, padding: 0}).after(e);
							b();
							e.append("<ul>" + w + "</ul>");
							var m = c("ul", e).css({position: "relative", "overflow-x": "hidden",
								"-webkit-overflow-scrolling": "touch"}), k = c("li", e).attr("unselectable", "on").css({"-webkit-user-select": "none", "-moz-user-select": "none", "-ms-user-select": "none", "-o-user-select": "none", "user-select": "none", "white-space": "nowrap"}), d = a.attr("size"), h = m.outerHeight(), f = k.outerHeight();
							void 0 !== d && 0 < d ? m.css({height: f * d}) : m.css({height: 4 * f});
							h > e.height() && (m.css("overflowY", "scroll"), q(m), k.filter(".selected").length && m.scrollTop(m.scrollTop() + k.filter(".selected").position().top));
							a.prependTo(e).css({position: "absolute",
							...
							a.is(":disabled") ? (e.addClass("disabled"), g.each(function () {
								...
								if (b.ctrlKey || b.metaKey)d.is(".selected") ? d.removeClass("selected first") :
									d.addClass("selected first"), d.siblings().removeClass("first");
								if (b.shiftKey) {
									var f = !1, h = !1;
